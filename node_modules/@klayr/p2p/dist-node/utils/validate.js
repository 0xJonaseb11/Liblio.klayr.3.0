...
const validatePeerAddress = (ipAddress, port) => {
    if (!(0, validator_1.isIPV4)(ipAddress) || !(0, validator_1.isPort)(port.toString())) {
        return false;
    }
    return true;
};
...
const validatePeerInfo = (peerInfo, maxByteSize) => {
    ...
    if (!peerInfo.ipAddress ||
        !peerInfo.port ||
        !(0, exports.validatePeerAddress)(peerInfo.ipAddress, peerInfo.port)) {
    ...
    const byteSize = (0, _1.getByteSize)(peerInfo);
...
const validatePayloadSize = (nodeInfo, maxByteSize) => {
    ...
    if ((0, _1.getByteSize)(nodeInfo) > maxByteSize) {
...
const validatePeerInfoList = (peersList, maxPeerInfoListLength, maxPeerInfoByteSize) => {
    ...
    peersList.map(peerInfo => (0, exports.validatePeerInfo)(peerInfo, maxPeerInfoByteSize));
};
exports.validatePeerInfoList = validatePeerInfoList;
const validateRPCRequest = (request) => {
    try {
        validator_1.validator.validate(schemas_1.rpcRequestSchema, request);
    }
    catch {
...
const validateProtocolMessage = (message) => {
    try {
        validator_1.validator.validate(schemas_1.protocolMessageSchema, message);
    }
    catch {
...
const validatePacket = (packet) => {
    try {
        validator_1.validator.validate(schemas_1.packetSchema, packet);
    }
    catch {
