...
const generateHash = (prefix, leftHash, rightHash) => cryptography_1.utils.hash(Buffer.concat([prefix, leftHash, rightHash], prefix.length + leftHash.length + rightHash.length));
...
const calculateMerkleRoot = ({ value, appendPath, size }) => {
    ...
    const newLeafHash = cryptography_1.utils.hash(leafValueWithPrefix);
...
const calculateMerkleRootWithLeaves = (data) => {
    ...
    if (data.length === 1) {
        ...
        return cryptography_1.utils.hash(leafValueWithPrefix);
    }
