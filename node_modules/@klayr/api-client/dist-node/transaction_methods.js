...
class TransactionMethods {
    ...
    async create(input, privateKeyHex) {
        ...
        const address = cryptography_1.address.getKlayr32AddressFromPublicKey(publicKey);
        let authAccount;
        try {
        ...
        const commandSchema = (0, codec_2.getTransactionParamsSchema)(txInput, this._metadata);
        const rawTx = {
            ...txInput,
            ...
            params: commandSchema
                ? codec_1.codec.fromJSON(commandSchema, txInput.params)
                : {},
        };
        if (authAccount.numberOfSignatures > 0) {
            const signedTx = (0, transactions_1.signMultiSignatureTransaction)(rawTx, chainID, privateKey, {
        ...
        const signedTx = (0, transactions_1.signTransaction)(rawTx, chainID, privateKey, commandSchema);
        return this.toJSON(signedTx);
    }
    async get(id) {
    ...
    async sign(transaction, privateKeyHexes) {
        const commandSchema = (0, codec_2.getTransactionParamsSchema)(transaction, this._metadata);
        const decodedTx = this.fromJSON(transaction);
        this._validateTransaction(decodedTx);
        const chainID = Buffer.from(this._nodeInfo.chainID, 'hex');
        const address = cryptography_1.address.getKlayr32AddressFromPublicKey(decodedTx.senderPublicKey);
        ...
        if (authAccount.numberOfSignatures > 0) {
            for (const privateKeyHex of privateKeyHexes) {
                const privateKey = Buffer.from(privateKeyHex, 'hex');
                (0, transactions_1.signMultiSignatureTransaction)(decodedTx, chainID, privateKey, {
                    mandatoryKeys: authAccount.mandatoryKeys.map(k => Buffer.from(k, 'hex')),
                    optionalKeys: authAccount.optionalKeys.map(k => Buffer.from(k, 'hex')),
                }, commandSchema);
            }
        ...
        const signedTx = (0, transactions_1.signTransaction)(decodedTx, chainID, Buffer.from(privateKeyHexes[0], 'hex'), commandSchema);
        return this.toJSON(signedTx);
    }
    async send(transaction) {
        const decodedTx = this.fromJSON(transaction);
        this._validateTransaction(decodedTx);
        const encodedTx = (0, codec_2.encodeTransaction)(decodedTx, this._schema, this._metadata);
    ...
    decode(transaction) {
        ...
        return (0, codec_2.decodeTransaction)(transactionBuffer, this._schema, this._metadata);
    }
    encode(transaction) {
        this._validateTransaction(transaction);
        return (0, codec_2.encodeTransaction)(transaction, this._schema, this._metadata);
    }
    computeMinFee(transaction, options) {
        const decodedTx = this.fromJSON(transaction);
        this._validateTransaction(decodedTx);
        const commandSchema = (0, codec_2.getTransactionParamsSchema)(transaction, this._metadata);
        return (0, transactions_1.computeMinFee)(decodedTx, commandSchema, options);
    }
    toJSON(transaction) {
        this._validateTransaction(transaction);
        return (0, codec_2.toTransactionJSON)(transaction, this._schema, this._metadata);
    }
    fromJSON(transaction) {
        return (0, codec_2.fromTransactionJSON)(transaction, this._schema, this._metadata);
    }
    _validateTransaction(transaction) {
        ...
        validator_1.validator.validate(this._schema.transaction, {
            ...rest,
