...
const decodeTransactionParams = (transaction, metadata) => {
    ...
    return {
        ...transaction,
        params: codec_1.codec.decode(paramsSchema, transaction.params),
    };
};
...
const decodeEventData = (event, metadata) => {
    ...
    return {
        ...event,
        data: eventDataSchema ? codec_1.codec.decodeJSON(eventDataSchema, Buffer.from(event.data, 'hex')) : {},
    };
};
exports.decodeEventData = decodeEventData;
const decodeTransaction = (encodedTransaction, registeredSchema, metadata) => {
    const transaction = codec_1.codec.decode(registeredSchema.transaction, encodedTransaction);
    const paramsSchema = (0, exports.getTransactionParamsSchema)(transaction, metadata);
    const params = codec_1.codec.decode(paramsSchema, transaction.params);
    const id = cryptography_1.utils.hash(encodedTransaction);
    return {
        ...transaction,
        params,
        id,
...
const encodeTransaction = (transaction, registeredSchema, metadata) => {
    ...
    if (!Buffer.isBuffer(transaction.params)) {
        const paramsSchema = (0, exports.getTransactionParamsSchema)(transaction, metadata);
        encodedParams = codec_1.codec.encode(paramsSchema, transaction.params);
    }
    else {
        encodedParams = transaction.params;
    }
    const decodedTransaction = codec_1.codec.encode(registeredSchema.transaction, {
        ...transaction,
...
const fromTransactionJSON = (transaction, registeredSchema, metadata) => {
    const paramsSchema = (0, exports.getTransactionParamsSchema)(transaction, metadata);
    const tx = codec_1.codec.fromJSON(registeredSchema.transaction, {
        ...transaction,
        params: '',
    });
    let params;
    if (typeof transaction.params === 'string') {
        params = codec_1.codec.decode(paramsSchema, Buffer.from(transaction.params, 'hex'));
    }
    else {
        params = codec_1.codec.fromJSON(paramsSchema, transaction.params);
    }
    return {
        ...tx,
...
const toTransactionJSON = (transaction, registeredSchema, metadata) => {
    ...
    if (Buffer.isBuffer(transaction.params)) {
        return {
            ...codec_1.codec.toJSON(registeredSchema.transaction, transaction),
            params: codec_1.codec.decodeJSON(paramsSchema, transaction.params),
            id: transaction.id.toString('hex'),
        };
    }
    return {
        ...codec_1.codec.toJSON(registeredSchema.transaction, {
            ...transaction,
            params: Buffer.alloc(0),
        }),
        params: codec_1.codec.toJSON(paramsSchema, transaction.params),
        id: transaction.id.toString('hex'),
    };
};
exports.toTransactionJSON = toTransactionJSON;
const decodeAssets = (blockVersion, encodedAssets, registeredSchema, metadata) => {
    const assets = encodedAssets.map(asset => codec_1.codec.decode(registeredSchema.asset, asset));
    const decodedAssets = [];
    for (const asset of assets) {
        const assetSchema = (0, exports.getAssetDataSchema)(blockVersion, { module: asset.module }, metadata);
        const decodedData = codec_1.codec.decode(assetSchema, asset.data);
        decodedAssets.push({
            ...asset,
            data: decodedData,
        });
...
const encodeAssets = (blockVersion, assets, registeredSchema, metadata) => {
    ...
    for (const asset of assets) {
        ...
        if (!Buffer.isBuffer(asset.data)) {
            ...
            encodedData = codec_1.codec.encode(dataSchema, asset.data);
        }
        else {
            encodedData = asset.data;
        }
        const encodedAsset = codec_1.codec.encode(registeredSchema.asset, {
            ...asset,
            data: encodedData,
        });
...
const fromBlockAssetJSON = (blockVersion, asset, registeredSchema, metadata) => {
    ...
    if (typeof asset.data === 'string') {
        return {
            ...codec_1.codec.fromJSON(registeredSchema.asset, asset),
            data: codec_1.codec.decode(dataSchema, Buffer.from(asset.data, 'hex')),
        };
    }
    return {
        ...codec_1.codec.fromJSON(registeredSchema.asset, { ...asset, data: '' }),
        data: codec_1.codec.fromJSON(dataSchema, asset.data),
    };
};
...
const toBlockAssetJSON = (blockVersion, asset, registeredSchema, metadata) => {
    ...
    if (Buffer.isBuffer(asset.data)) {
        return {
            ...codec_1.codec.toJSON(registeredSchema.asset, asset),
            data: codec_1.codec.decodeJSON(dataSchema, asset.data),
        };
    }
    return {
        ...codec_1.codec.toJSON(registeredSchema.asset, { ...asset, data: Buffer.alloc(0) }),
        data: codec_1.codec.toJSON(dataSchema, asset.data),
    };
};
exports.toBlockAssetJSON = toBlockAssetJSON;
const decodeBlock = (encodedBlock, registeredSchema, metadata) => {
    const block = codec_1.codec.decode(registeredSchema.block, encodedBlock);
    const header = codec_1.codec.decode(registeredSchema.header, block.header);
    const id = cryptography_1.utils.hash(block.header);
    const transactions = [];
...
const encodeBlock = (block, registeredSchema, metadata) => {
    ...
    const encodedBlockHeader = codec_1.codec.encode(registeredSchema.header, block.header);
    return codec_1.codec.encode(registeredSchema.block, {
...
const fromBlockJSON = (block, registeredSchema, metadata) => {
    const header = codec_1.codec.fromJSON(registeredSchema.header, block.header);
    const id = cryptography_1.utils.hash(codec_1.codec.encode(registeredSchema.header, header));
    const transactions = [];
...
const toBlockJSON = (block, registeredSchema, metadata) => {
    const header = codec_1.codec.toJSON(registeredSchema.header, block.header);
    const transactions = [];
    for (const transaction of block.transactions) {
...
const decodeBlockJSON = (block, metadata) => {
    ...
    for (const transaction of block.transactions) {
        ...
        const paramsSchema = (0, exports.getTransactionParamsSchema)(transaction, metadata);
        const paramsJSON = codec_1.codec.decodeJSON(paramsSchema, params);
        transactions.push({
            ...transaction,
            params: paramsJSON,
        });
    ...
    for (const asset of block.assets) {
        ...
        assets.push({
            ...asset,
            data: codec_1.codec.decodeJSON(dataSchema, Buffer.from(asset.data, 'hex')),
        });
    }
    return {
        header: block.header,
