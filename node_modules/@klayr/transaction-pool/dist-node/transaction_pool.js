...
const debug = createDebug('klayr:transaction_pool');
...
class TransactionPool {
    constructor(config) {
        ...
        this._feePriorityQueue = new utils_1.dataStructures.MinHeap();
        this._allTransactions = new utils_1.dataStructures.BufferMap();
        this._transactionList = new utils_1.dataStructures.BufferMap();
    ...
    async add(incomingTx) {
        ...
        const incomingTxAddress = cryptography_1.address.getAddressFromPublicKey(incomingTx.senderPublicKey);
        ...
        this.events.emit(exports.events.EVENT_TRANSACTION_ADDED, { transaction: incomingTx });
        return { status: types_1.Status.OK };
    }
    remove(tx) {
        var _a;
        ...
        const senderId = cryptography_1.address.getAddressFromPublicKey(foundTx.senderPublicKey);
    ...
    getProcessableTransactions() {
        const processableTransactions = new utils_1.dataStructures.BufferMap();
    ...
    _evictUnprocessable() {
        const unprocessableFeePriorityHeap = new utils_1.dataStructures.MinHeap();
        for (const txList of this._transactionList.values()) {
    ...
    _evictProcessable() {
        const processableFeePriorityHeap = new utils_1.dataStructures.MinHeap();
        for (const txList of this._transactionList.values()) {
        ...
        for (const txList of this._transactionList.values()) {
            ...
            for (let i = 0; i < allTransactions.length; i += 1) {
                ...
                try {
                    ...
                    if (status === types_1.TransactionStatus.INVALID) {
                        break;
                    }
                    if (status === types_1.TransactionStatus.UNPROCESSABLE) {
                    ...
                    verifyResults.push({
                        id: transaction.id,
                        status: types_1.TransactionStatus.INVALID,
                        transaction,
                    });
                    break;
                }
            ...
            const firstInvalidTransaction = verifyResults.find(r => r.status === types_1.TransactionStatus.INVALID);
