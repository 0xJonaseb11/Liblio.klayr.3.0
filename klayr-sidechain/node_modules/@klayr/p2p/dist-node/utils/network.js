...
const getIPBytes = (address) => {
    ...
    aBytes.writeUInt8((0, exports.getIPGroup)(address, 0), 0);
    const bBytes = Buffer.alloc(PREFIX_BUFFER_LENGTH);
    bBytes.writeUInt8((0, exports.getIPGroup)(address, 1), 0);
    const cBytes = Buffer.alloc(PREFIX_BUFFER_LENGTH);
    cBytes.writeUInt8((0, exports.getIPGroup)(address, 2), 0);
    const dBytes = Buffer.alloc(PREFIX_BUFFER_LENGTH);
    dBytes.writeUInt8((0, exports.getIPGroup)(address, 3), 0);
    return {
        aBytes,
        bBytes,
        cBytes,
...
const isPrivate = (address) => (0, exports.getIPGroup)(address, 0) === 10 ||
    ((0, exports.getIPGroup)(address, 0) === 172 && (0, exports.getIPGroup)(address, 1) >= 16 && (0, exports.getIPGroup)(address, 1) <= 31);
exports.isPrivate = isPrivate;
const isLocal = (address) => (0, exports.getIPGroup)(address, 0) === 127 || address === '0.0.0.0';
exports.isLocal = isLocal;
const getNetwork = (address) => {
    if ((0, exports.isLocal)(address)) {
        return NETWORK.NET_LOCAL;
    }
    if ((0, exports.isPrivate)(address)) {
...
const getNetgroup = (address, secret) => {
    ...
    return cryptography_1.utils.hash(netgroupBytes).readUInt32BE(0);
};
...
const getBucketId = (options) => {
    ...
    const secretBytes = Buffer.alloc(exports.SECRET_BUFFER_LENGTH);
    secretBytes.writeUInt32BE(secret, 0);
    const network = (0, exports.getNetwork)(targetAddress);
    const networkBytes = Buffer.alloc(exports.NETWORK_BUFFER_LENGTH);
    networkBytes.writeUInt8(network, 0);
    const { aBytes: targetABytes, bBytes: targetBBytes, cBytes: targetCBytes, dBytes: targetDBytes, } = (0, exports.getIPBytes)(targetAddress);
    if (network === NETWORK.NET_OTHER) {
        throw Error('IP address is unsupported.');
    }
    if (network !== NETWORK.NET_IPV4) {
        return cryptography_1.utils.hash(Buffer.concat([secretBytes, networkBytes])).readUInt32BE(0) % bucketCount;
    }
    const addressBytes = Buffer.concat([targetABytes, targetBBytes, targetCBytes, targetDBytes]);
    const kBytes = Buffer.alloc(firstMod);
    const sourceBytes = sourceAddress ? (0, exports.getIPBytes)(sourceAddress) : undefined;
    const k = peerType === PEER_TYPE.NEW_PEER && sourceBytes
        ? cryptography_1.utils
            .hash(Buffer.concat([
            secretBytes,
            networkBytes,
            sourceBytes.aBytes,
        ...
        : cryptography_1.utils.hash(Buffer.concat([secretBytes, networkBytes, addressBytes])).readUInt32BE(0) %
            firstMod;
    ...
    return cryptography_1.utils.hash(bucketBytes).readUInt32BE(0) % bucketCount;
};
