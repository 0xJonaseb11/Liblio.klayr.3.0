...
class Peer extends events_1.EventEmitter {
    constructor(peerInfo, peerConfig) {
        ...
        this._handleRawRPC = (packet, respond) => {
            try {
                (0, utils_1.validateRPCRequest)(packet);
            }
            catch (error) {
                respond(error);
        ...
        this._handleRawMessage = (packet) => {
            try {
                (0, utils_1.validateProtocolMessage)(packet);
            }
            catch (error) {
    ...
    async fetchPeers() {
        try {
            ...
            const { peers } = codec_1.codec.decode(this._rpcSchemas.peerRequestResponse, response.data);
            const sanitizedPeersList = peers.map((peerInfoBuffer) => {
                const peerInfo = codec_1.codec.decode(this._rpcSchemas.peerInfo, peerInfoBuffer);
                return (0, utils_1.sanitizeIncomingPeerInfo)(peerInfo);
            });
            (0, utils_1.validatePeerInfoList)(sanitizedPeersList, this._peerConfig.maxPeerDiscoveryResponseLength, this._peerConfig.maxPeerInfoSize);
    ...
    async fetchAndUpdateStatus() {
        ...
        try {
            const receivedNodeInfo = (0, codec_2.decodeNodeInfo)(this._rpcSchemas.nodeInfo, response.data);
    ...
    _updateFromProtocolPeerInfo(rawPeerInfo) {
        ...
        const peerInfo = (0, utils_1.validatePeerInfo)((0, utils_1.sanitizeIncomingPeerInfo)({
            ...rawPeerInfo,
            ipAddress: this.ipAddress,
            port: this.port,
        }), this._peerConfig.maxPeerInfoSize);
        const result = (0, utils_1.validatePeerCompatibility)(peerInfo, this._serverNodeInfo);
    ...
    _handleUpdateNodeInfo(message) {
        var _a;
        try {
            (0, utils_1.validatePayloadSize)(message.data, this._peerConfig.maxPeerInfoSize);
            const decodedNodeInfo = (0, codec_2.decodeNodeInfo)(this._rpcSchemas.nodeInfo, message.data);
    ...
    _initializeInternalState(peerInfo) {
        return peerInfo.internalState
            ...
            : {
                ...peerInfo,
                internalState: (0, utils_1.assignInternalInfo)(peerInfo, this._peerConfig.secret),
            };
    }
    _getBase64Data(data) {
        if (data === undefined) {
