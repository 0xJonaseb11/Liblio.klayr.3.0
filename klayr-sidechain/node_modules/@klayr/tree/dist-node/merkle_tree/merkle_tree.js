...
class MerkleTree {
    ...
    async update(idxs, updateData) {
        ...
        for (const data of updateData) {
            ...
            const leafHash = cryptography_1.utils.hash(leafValueWithoutNodeIndex);
    ...
    async _generateLeaf(value, nodeIndex) {
        ...
        const leafHash = this._preHashedLeaf ? value : cryptography_1.utils.hash(leafValueWithoutNodeIndex);
    ...
    async _getNode(nodeHash) {
        ...
        const rightHash = type === "branch" ? value.subarray(-1 * constants_1.NODE_HASH_SIZE) : Buffer.alloc(0);
        const leftHash = type === "branch"
            ? value.subarray(-2 * constants_1.NODE_HASH_SIZE, -1 * constants_1.NODE_HASH_SIZE)
            : Buffer.alloc(0);
        return {
            type,
            hash: nodeHash,
